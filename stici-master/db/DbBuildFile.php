...
class DbBuildFile
{
	...
	public static function GetJobFiles($job_id, $limit = 50)
	{
		$list = array();
	
		$con = new DbConnection();
	
		$query = "SELECT bf.file_id, bf.build_id, bf.filepath, bf.filename, bf.filesize, bf.file_hash, bf.stamp, b.job_id FROM build_files bf JOIN builds b ON b.build_id = bf.build_id WHERE b.job_id = ? ORDER BY bf.file_id DESC LIMIT ?";
	...
	public static function GetLastFiles($limit = 50)
	{
		$list = array();
		
		$con = new DbConnection();
		
		$query = "SELECT bf.file_id, bf.build_id, bf.filepath, bf.filename, bf.filesize, bf.file_hash, bf.stamp, b.job_id, j.job_name FROM build_files bf JOIN builds b ON b.build_id = bf.build_id JOIN jobs j ON j.job_id = b.job_id ORDER BY bf.file_id DESC LIMIT ?";
		
		$st = $con->prepare($query);
		$st->bind_param("i", $limit);
		$st->bind_result($f_id, $b_id, $f_path, $f_name, $f_size, $f_hash, $f_stamp, $j_id, $j_name);
		$st->execute();
